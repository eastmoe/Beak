...
- `semantic_markdown` —— 浏览器可见语义 DOM 提取后转 Markdown
- `semantic_jsonl` —— 浏览器可见语义 DOM 提取后转 JSONL
- `accessibility_yaml` —— Playwright `aria_snapshot` 导出的 accessibility tree YAML（需要 `engine=edge`）
...
```powershell
pip install beak-0.0.3-py3-none-win_amd64.whl
```
...
语义 DOM 与 accessibility tree 导出：

```powershell
Invoke-RestMethod http://127.0.0.1:8000/render `
  -Method Post -ContentType "application/json" `
  -Body '{"url":"https://example.com","engine":"webview","output":"semantic_markdown"}'

Invoke-RestMethod http://127.0.0.1:8000/render `
  -Method Post -ContentType "application/json" `
  -Body '{"url":"https://example.com","engine":"edge","output":"accessibility_yaml"}'
```
...
Beak 启动时会读取本地 JSON 配置文件。如果没有指定配置文件，首次启动会自动创建并沿用用户目录下的 `~/.beak/config.json`。也可以用环境变量指定其它路径：

```powershell
$env:BEAK_CONFIG_FILE = "D:\Beak\beak.config.json"
beak server
```

WebUI 可以直接编辑当前配置文件，保存后热加载生效。

配置文件可以按域名匹配规则提供默认请求参数。只有 API 请求里没有显式传入的字段，才会从匹配规则里补上：
...
```json
{
  ...
  "safety": {
    ...
    "block_loopback_hosts": true,
    "block_link_local_hosts": true,
    "block_multicast_hosts": true,
    "resolve_dns": true,
    "max_url_length": 2048
  }
}
...
`patterns` 支持精确域名、`*.example.com` 后缀匹配，也支持普通 `*` 通配。安全策略默认关闭；开启后会在任务进入队列或同步执行前拦截不符合策略的 URL。
...
│       ├── config.py           # 本地配置、域名默认值和 URL 安全策略
