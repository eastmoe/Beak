...
Beak is a Windows browser-rendering crawler service. The HTTP layer is Python + FastAPI, while page loading and export are delegated to an isolated Microsoft Edge WebView2 worker process.

## Features

- `POST /render` and `POST /capture` APIs with FastAPI OpenAPI docs at `/docs`.
- Per-task URL, timeout, wait strategy, proxy, cookies, User-Agent, viewport and output format.
- Output types:
...
- Proxy isolation by launching each task in a separate WebView2 worker with its own user data directory.
...
The current machine did not have `dotnet` on PATH when this project was scaffolded, so the Python service can be syntax-checked here but the WebView2 worker must be published after installing the .NET SDK.
...
Proxy support is process-level. Beak starts a separate WebView2 worker/user-data directory per task so `--proxy-server` does not leak across concurrent jobs.

## Notes

WebView2 does not expose every Microsoft Edge "Save page as..." behavior as a simple stable API. Beak uses WebView2-native capture for screenshots, DevTools `Page.captureSnapshot` for `single_file` MHTML, and DevTools network bodies plus rendered DOM rewriting for `complete_page`. Some cross-origin or cache-only resources may not be available from DevTools after page load; the ZIP still includes the rendered HTML and every captured response body available to WebView2.
